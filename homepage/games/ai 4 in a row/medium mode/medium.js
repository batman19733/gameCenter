...
function minimax(depth, isMaximizing) {
    ...
    if (isMaximizing) {
        ...
        for (let [row, col] of nums) {
            ...
            if(detectWinBy('b', 'b', 'b', 'b')) {console.log('you could of won!')} 
            grid[row][col] = ''
            if (score < bestScore) {
                bestScore = score
            }
...
function goDownGrid(row, col) {
    if (row + 1 < index) {if (grid[row + 1][col] === '') {return goDownGrid(row + 1, col)} else {return [row, col]}} else {return [row, col]} 
}
function q(qury) {
